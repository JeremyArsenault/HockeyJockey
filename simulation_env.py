...
class AirHockey(HockeyPhysics):
    ...
    def reset(self):
        ...
        puck_vy = np.random.uniform(0.1, 0.4)*(2*np.random.randint(2)-1)
        ...
        ob1, ob2 = self.get_observations()
        return ob1, ob2
        
    def step(self, s1_action, s2_action):
        """
        Take action and step forward in time
        :return: observ1, reward1, observ2, reward2, done, info
        info: true if draw
        """
...
class SinglePlayerReturn(HockeyPhysics):
    """
    OpenAI gym style air hockey env to simulate returning a air hockey shot
    """
    def __init__(self, action_space, freq=0.2, max_length=60):
        HockeyPhysics.__init__(self)
        self.consts['table_goal_width'] = self.consts['table_width']-2*self.consts['table_corner_radius']
        self.action_space = action_space
        self.freq = freq
        self.max_length = max_length
        self.reset()
    ...
    def reset(self):
        ...
        """
        self.time = 0
        striker_y = -self.consts['table_length']/2+np.random.uniform(0.1, 0.2)
        striker_x = np.random.uniform(-0.2,0.2)
        puck_vx = np.random.uniform(-0.5,0.5)
        puck_vy = np.random.uniform(-0.4, -0.1)
        puck_x = np.random.uniform(-0.2,0.2)
        puck_y = self.consts['table_length']/2-0.3
        self.state['puck']['position'] = np.array([puck_x, puck_y])
        self.state['puck']['velocity'] = np.array([puck_vx,puck_vy])
        self.state['striker1']['position'] = np.array([striker_x,striker_y])
        self.state['striker1']['velocity'] = np.zeros(2)
        self.state['striker2']['position'] = np.array([-striker_x,-striker_y])
    ...
    def step(self, action):
        ...
        v0 = self.state['striker1']['velocity']
        p = self.state['striker1']['position']
        v = self.action_space.execute(action, v0, p)
        self.state['striker1']['velocity'] = v
        ...
        o, _ = self.get_observations()
        r = self.is_goal() if self.time<=self.max_length else -1
        d = self.is_goal() or (self.time>self.max_length)
        i = None
